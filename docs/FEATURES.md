...
- AC:
  ...
  - [x] CTA href configurable vía i18n key `hero_announcement.cta_href`. Soporta interno `/...`, externo `tel:` / `mailto:` / `https://`. `isAllowedCtaHref` (server-side) rechaza esquemas no whitelisted + protocol-relative `//host` (XSS / open-redirect guard); cubierto por Vitest
...
- Notas:
  ...
  - **Desviación menor:** `cta_href` por defecto = `tel:+41766381870` (team building); cuando aterrice `/contacto` (F-054) el owner cambia la key i18n sin tocar código
