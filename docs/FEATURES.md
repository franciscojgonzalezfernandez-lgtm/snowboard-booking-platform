...
- Sprint: post-Sprint 5 (security hardening) · Estado: backlog · Prioridad: **P1 (seguridad — DoS de inventario de reservas)**
...
- AC — verificación de email (core):
  - [ ] Activar verificación para el path password: `emailAndPassword.requireEmailVerification: true` + `emailVerification.sendVerificationEmail` (React Email + Resend, i18n en/de/es como `send-magic-link-email`). Confirmar que Google (trusted, `email_verified`) y magic link (marca `emailVerified` al usar) quedan **exentos**.
  - [ ] **Defense-in-depth en el chokepoint del hold:** `createBookingDraftWith` rechaza con nuevo error `EMAIL_NOT_VERIFIED` si `session.user.emailVerified !== true`, **antes** de crear la fila `PENDING_PAYMENT` y el PaymentIntent. Independiente de la config de auth: aunque algún path deje una sesión sin verificar, el hold se niega. Añadir `emailVerified` al shape de sesión si falta.
  - [ ] Funnel Step 4 (bloque F-119, `step4-auth.tsx`): tras submit email+password de una cuenta **no verificada**, en vez de voltear a payment mostrar estado "verifica tu email para continuar" (con reenvío). Google/magic link siguen. → email+password deja de ser 100% on-page (round-trip al inbox, como el magic link) — **trade-off aceptado (D-2026-07-28)**.
- AC — límite de daño (defense-in-depth adicional):
  - [ ] Cap de holds `PENDING_PAYMENT` concurrentes por usuario (p.ej. máx 1–2): `createBookingDraft` rechaza si el booker ya tiene N holds vivos → un actor no acapara múltiples slots aunque verifique.
  - [ ] Revisar rate-limit de signup + de `createBookingDraft` (Better Auth rate-limita en prod; añadir por-IP si falta). Opcional: acortar TTL / subir cadencia de `expire-pending`.
- Tests:
  - [ ] Unit `create-draft`: sesión `emailVerified:false` → `EMAIL_NOT_VERIFIED`, **no** crea fila ni PaymentIntent; `emailVerified:true` → happy path intacto. Cap de holds concurrentes.
  - [ ] E2E: email+password nuevo en el funnel → estado "verifica tu email" (no payment); Google/magic → proceden; usuario verificado existente reserva sin fricción nueva.
- Notas:
