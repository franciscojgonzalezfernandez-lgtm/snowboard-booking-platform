...
- AC server:
  ...
  - [x] **User-cancel restore (cierra el loophole de refresco de expiración)** — extiende `lib/booking/cancel.ts` (F-058). Sin esto, gastar un credit a punto de expirar en una reserva y cancelarla (≥48h) acuñaría un credit nuevo a +1 año → expiración reseteable indefinidamente; además el overshoot todo-o-nada haría perder valor. Decisión (2026-05-28): **restaurar los credits originales en vez de re-acuñar**.
    - **Credit path** (`CONFIRMED` + ≥48h): dentro del `$transaction`, tras flipear el booking, `accountCredit.findMany({ where: { usedOnBookingId: booking.id, status: 'USED' } })` → flip `USED` → `ACTIVE`, clear `usedAt` + `usedOnBookingId`, **preservando `expiresAt` original**. `restoredCents = sum(amountCents)`. Credit fresco (1 año) **sólo** por la porción cash: `freshCreditCents = max(0, totalPriceCents - restoredCents)`; si `> 0`, `accountCredit.create`. Booking 100% cash (sin credits usados) → `restoredCents = 0` → un único credit de `totalPriceCents` (comportamiento F-058 intacto).
  ...
  - [ ] **Ops-refund path (Sprint 4)**: `cancelBookingByOps` reutilizará el mismo patrón restore + Stripe refund de la porción cash. Sprint 3 no implementa la rama ops; sólo deja el ledger preparado.
- AC seguridad:
  - [x] Constraint Postgres no necesario — `LOCKED` status + check de status en `where` del update suficiente. Race condition double-lock cubierta por el guard `status: ACTIVE` en el `updateMany`: si `count !== consumedIds.length` → `CreditConflictError` → rollback del `$transaction`.
...
- Tests:
  - [x] Vitest `lib/booking/create-draft.test.ts` extendido (+9 specs F-060): no-creditIds skip, happy partial, happy full (zero-charge), oldest-first cap, overshoot todo-o-nada, credit excluido por findMany mismatch (expirado/usado/lockeado/no-propio), race double-lock, reused draft refleja charge/credits del PI existente. Webhook idempotency (settle + release) en `handle-webhook.test.ts`; release por draft expirado en `expire-pending.test.ts`; restore/forfeit en `cancel.test.ts`. Total suite 242 verde.
  - [x] Playwright `e2e/f-060-credit-checkout.spec.ts` (en) — slots libres dinámicos: (1) credit `11000` cubre `ONE_HOUR` (CHF 110) → Step 4 `credit=auto` pre-selecciona → CTA `Confirm booking` → success page `CONFIRMED` + credit `USED`; (2) credit parcial `5000` → Step 5 breakdown `110 − 50 = 60` + Payment Element.
- Notas:
