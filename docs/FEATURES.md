...
> Sprint más grande del MVP: self-service del instructor + panel ops/admin. `/instructor` y `/admin` viven **fuera de `[locale]`** (EN-only, ver Routing conventions). GCal (F-074 OAuth + F-075 sync) **landed** tras provisionar el owner `ENCRYPTION_KEY` + consent screen (2026-06-05). **Sprint cerrado (2026-06-15):** los 11 tickets core mergeados — F-071, F-072, F-073, F-065, F-076, F-077, F-078, F-079, F-080, F-081, F-105 (season management). Único pendiente: ⛔ **F-082 Tip**, **aplazado** hasta que el owner formalice la empresa + provisione `INSTRUCTOR_TIP_URL` (TWINT) — fuera del scope del cierre de Sprint 4, se retoma post-constitución. F-105 (season management, **renumerado desde F-088** para liberar F-088 a la wave de marca "The Drop" del Sprint 5) añadido tras review de PR #118: F-080 lee la Season activa pero nada in-app la crea/activa.
>
...
Admin:       F-076 ─┬─ F-077 (bookings view) ── F-078 (ops-cancel) ── F-079 (cancel day)
                    ...
                    └─ F-105 (season CRUD + activate) ── precede a F-080 en operación real
             F-081 (no-show re-flip; después de F-077 + F-071)
Tip:         F-082 ⛔ (paralelo, blocked tail)
```

Critical path: **F-076 → F-077 → F-078 → F-079** (cadena ops-cancel) — **done**. La cadena instructor **F-071 → F-073 → F-074 → F-075** (GCal tail) — **done**. ⛔ = blocked-in-progress (solo F-082, **aplazado** a post-constitución de empresa).
...
##### F-105 — Season management admin UI (CRUD + activate/deactivate)

- Sprint: 4 · Estado: done (PR #133) · Prioridad: P1 — renumerado **F-087 → F-088 → F-105**: F-087 quedó para "Admin student directory" (#132), F-088 para la wave de marca Sprint 5 "The Drop" (#134); este ticket (season management) es **F-105**.
...
- Tests:
  ...
  - [ ] Playwright `e2e/f-105-seasons.spec.ts`: no-admin → 404; crear season → aparece en lista; activar season B → A pierde el badge (1 sola activa); activar con pricing incompleto → bloqueado + hint; con 0 activas `/admin/pricing` muestra el empty-state + CTA a seasons.
- Notas:
  ...
  - **Invariante "una sola activa"** es de facto hoy (el seed mantiene 1 row) pero **no** está enforced a nivel DB. F-105 lo enforced en el `activateSeason` `$transaction`. Un partial-unique-index (`@@unique` sobre `active` where `active=true`) es post-MVP si el owner quiere garantía a nivel DB — requiere migración + manejo del race.
  ...
  - **MVP single-instructor / single-active-season**: F-080 puede mergear sin F-105 (su empty-state es honesto sobre la precondición); F-105 desbloquea la operación real multi-temporada antes del segundo invierno.
...
- Sprint: 4 · Estado: **aplazado a post-constitución de empresa** (blocked-in-progress) · Prioridad: P2
...
- Notas:
  ...
  - **Aplazado (2026-06-15):** sacado del cierre de Sprint 4 por decisión del owner — se retoma cuando la empresa esté constituida y exista el medio de pago de propinas (`INSTRUCTOR_TIP_URL` / TWINT). El ticket queda con sus AC escritos para retomar sin re-contexto.
...
- AC:
  - [x] `lib/contact/phone.ts` exporta constantes `OPERATIONAL_PHONE_DISPLAY = "+41 76 638 18 70"` + `OPERATIONAL_PHONE_TEL = "+41766381870"`. Single source of truth; T&C / Privacy / email templates consumen las mismas constantes en lugar de re-escribir. (`_TEL` se deriva de `_DISPLAY` con `.replace(/\s+/g,"")` para que no puedan divergir.)
...
- Sprint: 5 (hotfix) · Estado: **done** (2026-07-12, PR pendiente) · Prioridad: P1 (bugs en producción, owner los reportó 2026-06-24)
...
- **Entregado (2026-07-12):**
  - Issue 1 — `app/[locale]/(marketing)/page.tsx`: lee la sesión server-side (`auth.api.getSession({ headers: await headers() })`, mismo patrón que `SiteNav`) y envuelve el `<Link href="/login">` en `{!session?.user && …}`. Se eligió el **AC mínimo (ocultar)**, no el CTA a `/dashboard` (evita añadir i18n `cta_dashboard` pendiente de copy del owner; el acceso al dashboard ya está en el `SiteNav` autenticado). Añadidos `data-testid="hero-cta-primary"`/`"hero-cta-signin"` para test inequívoco (el `SiteNav` también tiene un link de login → hacía falta acotar al hero). Home ya era dinámica (el layout monta `SiteNav` async) → sin coste nuevo de LCP.
  - Issue 2 — `app/[locale]/reservar/exito/[id]/page.tsx`: `isPast = setUtcTime(startOfUtcDay(booking.date), booking.anchorTime).getTime() <= Date.now()` (mismo cálculo que `booking-row.tsx`); el `<a exito-add-to-calendar>` ahora bajo `isConfirmed && !isPast`. El `exito-go-to-dashboard` permanece. Endpoint `/api/booking/[id]/ics` **no** tocado (gate de UI, no de API).
  - Tests: `e2e/f-107-stale-ctas.spec.ts` — issue 1 (anon /en·/de·/es muestran ambos CTAs; autenticado oculta `hero-cta-signin`, mantiene primary); issue 2 (futuro CONFIRMED muestra ICS; pasado COMPLETED **y** CONFIRMED-vencido ocultan ICS, mantienen dashboard). 5/5. Nota: el spec minimiza sign-ups (2 en total) porque Better Auth **aplica rate-limiting en builds de producción** (`next start`) — no en `npm run dev`, donde corre el e2e de CI; verificado localmente arrancando el server con `BETTER_AUTH_URL=http://localhost:3500` (si no, "Invalid origin" porque `.env.local` apunta a prod).
...
- Decisiones tomadas con el owner (2026-07-07):
  1. **String de marca:** `Ride Flumserberg` literal, en los 3 idiomas (no se traduce el nombre).
  2. **/about "Why the name":** se **elimina** la sección de origen del nombre (`name_title` + `name_body_1` + `name_body_2`). El párrafo del drop-moment (`name_body_2`, **sin** la frase final de origen "The name comes straight from it" / "Daher kommt der Name." / "De ahí sale el nombre.") se **mueve al cierre de "Why Flumserberg"** (`mountain_body`) como remate emocional. Una sección menos, página más corta.
...
- Ejecución (audit completado 2026-06-13; plan aprobado, 7 sub-tickets, un PR cada uno):
  ...
  - [x] **F-086b** — schemas: `lib/schemas/attendee.ts` compartido (hoy `draftAttendeeSchema` ≡ `attendeeSchema` de step4) + `lib/schemas/phone.ts` con regex E164 única (hoy 3 copias).
  - [x] **F-086c** — labels: `lib/labels/booking.ts` (STATUS/DURATION/LANGUAGE/LEVEL + credit maps tipados `Record<CreditReason|CreditStatus, string>`) y `lib/email/labels.ts` (dedupe de los 3 senders). Converge el drift de cancel-day ("Cancelled (user)" → "Cancelled · client"). Dashboard i18n-keys intactos.
...
- AC data layer:
  - [x] `lib/admin/students.ts` (deps-inyectado, patrón F-065/F-077):
  ...
  - [x] **Notas cross-instructor:** a diferencia de `getBookerNoteHistories` (scoped a un `instructorId`, F-065), aquí el admin ve las notas de **todos** los instructores para ese booker. Nueva query (o generalizar la de F-065 con `instructorId` opcional) con `status: COMPLETED` + `instructorNote: { not: null }`, `orderBy date desc`, **incluyendo el autor** (`instructor.user.name`) — multi-instructor lo exige; no hardcodear "Javi".
  - [x] **Credit balance helper:** no existe hoy (`lib/credit/` sólo tiene `expire.ts`). Extraer `lib/credit/balance.ts` `getActiveCreditCents(userId)` (sum `amountCents` where `status = ACTIVE`) — reusable por el credits-aside del dashboard (F-059) para no duplicar.
  - [x] **Lifetime stats:** lessons count = bookings `COMPLETED`; total spend = suma `totalPriceCents` de `CONFIRMED` + `COMPLETED` (excluye cancelled/refunded/pending). Documentar la regla inline.
- AC UI:
  - [x] `app/admin/students/page.tsx` (Server Component, EN-only, bajo `requireAdmin` via `app/admin/layout.tsx`): tabla name/email/phone + bookings count + last booking + active credit. Search box + paginación (reusar `app/admin/bookings/_components/{pagination,filter-bar}.tsx`). Row → `/admin/students/[id]`.
  - [x] `app/admin/students/[id]/page.tsx`: perfil con 4 secciones — (1) **Contact header** (name, email, phone, locale, roles); (2) **Lifetime stats** (lessons count, total spend CHF via `Intl.NumberFormat('de-CH')`, active credit); (3) **Notes timeline** read-only, newest-first, cada nota con fecha de la clase + `setAt` + autor (instructor); (4) **Bookings history** (todos los status, date/time/duration/instructor/status/price + attendees). 404 si el `User` no existe; empty states si existe sin bookings/notes.
  - [x] Añadir entrada **"Students"** al nav del admin shell (F-076) junto a Bookings/Instructors/Pricing.
  - [x] **Read-only:** NO montar `InstructorNoteField` (client island F-065) aquí — render como texto plano.
  - [x] Reusar `<StatusBadge>` si F-085 ya aterrizó; si no, status como label simple (no bloquear en F-085).
- AC seguridad / i18n:
  - [x] EN-only (admin fuera de `[locale]`) — sin next-intl.
  - [x] `requireAdmin()` cubierto por el layout; read-only (sin server actions). Confirmar que ninguna ruta filtra PII (email/phone) ni notas internas a no-admin.
  - [x] `instructorNote` es **internal-only** (F-065): admin-only OK. Nunca exponer en API pública ni en el dashboard del booker.
- Tests:
  - [x] Vitest `lib/admin/students.test.ts`: `listStudents` (paginación, search name/email, count + last booking + active credit correctos) y `getStudentProfile` (agrega bookings de todos los status; notes sólo de `COMPLETED` con note no-null e incluyen autor; spend = suma `CONFIRMED`+`COMPLETED`; credit balance = sum `ACTIVE`). Mock Prisma deps-inyectado.
  - [x] Playwright `e2e/f-087-admin-students.spec.ts`: admin → `/admin/students` lista al booker seed; click → perfil muestra bookings + notes timeline (las de F-065); search filtra; non-admin (student) → 403/redirect; **regresión cross-surface**: el booker NO ve esas notas en `/dashboard`.
  - [x] Fixtures: extender `prisma/seed.ts` con un "alumno con historial real" (booker + completed bookings + notes) — followup ya anotado en notas de F-021. Mientras tanto reusar el patrón de inyección manual usado para probar F-065.
- Notas:
...
- Sprint: post-Sprint 5 · Estado: **done** (2026-07-18, PR pendiente) · Prioridad: P2 (SEO surface + info, pedido por owner)
...
- **Entregado (2026-07-18):**
  - Ruta `app/[locale]/(marketing)/plan-your-visit/page.tsx` (server, `revalidate=3600` porque lee la `Season`). Slug traducido en `i18n/routing.ts`: `/plan-your-visit` · `/plane-deinen-besuch` · `/planea-tu-visita`. `generateStaticParams` + `setRequestLocale` + `generateMetadata` (`marketingAlternates` → canonical self + hreflang). Slug no-canónico → 307 (F-102, verificado).
  - **Season block LIVE** vía `lib/season/plan-status.ts` (`seasonStatus(window, now)` puro, comparación a nivel día UTC porque `@db.Date`): active → "Open until X"; upcoming → "Next season X → Y"; none → fallback. Fecha formateada por locale (`Intl.DateTimeFormat`, `timeZone:UTC`). Hoy (2026-07-18) renderiza el estado **upcoming** correcto.
  - Getting-here reusa `MEETING_POINT_LABEL` (`lib/contact/location`) + cross-link a `/contacto` (mapa allí, no duplicado). Gear-rental + resort-hours: links salientes `follow` (`rel="noopener"`, `target="_blank"`) a `flumserberg.ch` / `.../Operating-hours` (`lib/content/plan-your-visit.ts`). Añadida a `sitemap.ts` (hreflang) + **link en el footer** (`footer.plan_link` × 3).
  - **Schema.org:** ninguno nuevo (sin `openingHoursSpecification`/geo falsos — SAB/F-112). Hereda el `LocalBusiness` del layout marketing (F-100).
  - Tests: Playwright `e2e/f-115-plan-your-visit.spec.ts` (5): render × 3 locales + estado de temporada (`/202\d/`) + links salientes (`href`+`rel`+`target`) + footer-link navega + slug no-canónico 307. Vitest `lib/season/plan-status.test.ts` (5: upcoming/active/last-day/ended/none). Verificado además: canonical+hreflang en `<head>`, 3 slugs en el sitemap, **axe 0 critical/serious**, revisión visual editorial OK.
  - **Desviación honesta del AC (listados locales):** NO se inventan negocios de terceros (tiendas/restaurantes con nombre) — sería contenido no verificado en producción. Se envían sólo referencias verificables (el resort). La sección "gear rental / local guide" con negocios reales que el owner conozca queda como **followup de contenido curado por el owner** (el ticket ya lo anticipaba: "empezar con lo que el owner conozca").
  - **Nav "More":** el link va al footer por ahora; el emplazamiento en el menú "More" del nav es de **F-116** (no construido aún) para no empeorar el crowding que F-116 arregla.
...
- Sprint: 5 (funnel UX) · Estado: **done** (2026-07-24) · Prioridad: P2 (fricción de conversión — pedido por owner)
...
- Decisión (sesión 2026-07-21, con owner):
  ...
  - **Auto-provisión, sin paso de signup** — sin toggle signin/signup dentro del funnel. Google y magic link ya auto-crean cuenta en primer uso (Better Auth default). Email+password: **submit unificado**. **Orden invertido respecto al plan original** (`signIn`-first → `signUp` on-not-found) durante la implementación: Better Auth devuelve un error *ambiguo* ("invalid credentials") tanto para password incorrecta como para usuario inexistente en `signIn.email`, así que un signin-first no puede distinguir los casos. El flujo entregado es **`signUp.email` primero → si `code === "USER_ALREADY_EXISTS"`, cae a `signIn.email`** con las mismas credenciales. Esto provisiona limpio al usuario nuevo y hace que una password incorrecta de un usuario existente aflore como error de signin real (no como el confuso "already exists"). `name` derivado del email (`email.split("@")[0]`). Un solo affordance "continuar".
- AC — componente:
  - [x] Nuevo `app/[locale]/reservar/step4-auth.tsx` (`'use client'`), bespoke/compacto, montado en `page.tsx` en lugar del bloque `!session?.user` actual. Reemplaza el `<Link>`→`/login`; **no** deja ningún enlace a `/login` en el Step 4 (`Link` de `@/i18n/navigation` retirado del import de `page.tsx`).
  - [x] Los tres métodos: botón Google (`authClient.signIn.social`), magic link (`authClient.signIn.magicLink`, con estado "revisa tu email" inline), y form email+password (submit unificado signup-first→signin-on-exists).
  - [x] `callbackURL` = URL actual del funnel (reutiliza `buildLoginNext(locale, sp)`), para que Google/magic link vuelvan al Step 4 con el draft intacto (el draft vive en URL-state, sobrevive el round-trip).
  - [x] Tras éxito de **email+password** (único no-nav): `router.refresh()` (sin `push`) → el RSC re-evalúa `session?.user` y el Step 4 se voltea in-place a `BookerPaymentFlow`. Cero navegación de extremo a extremo. **Verificado por E2E** (`section-4` aparece, `section-4-anonymous` desaparece, pathname sigue `/en/reservar`).
  - [x] Diseño Impeccable: sin tabs; jerarquía editorial que casa con `booking-header`. shadcn `Button`/`Input`/`Form`. Screenshot desktop + móvil revisados (borde no sombra, Archivo Black uppercase, botones full-width, targets táctiles OK).
- AC — i18n:
  - [x] Copy vía next-intl en los 3 locales (en/de/es). **Reutiliza el namespace `login`** client-side (`useTranslations("login")`, igual que `login-form.tsx`) para Google/magic/email/password/errores/validación; **una** clave nueva `login.continue` (Continue/Weiter/Continuar) para el botón unificado. `anonymous_heading`/`anonymous_body` conservados (framing sigue válido); `anonymous_cta` retirado (ya no se referencia). Sin strings hardcodeados.
- AC — perf (`booking-platform-perf`):
  - [x] El bloque de auth solo pesa cuando el booker anónimo alcanza el Step 4 → **`next/dynamic`** (`const Step4Auth = dynamic(() => import("./step4-auth").then(m => m.Step4Auth))`) mantiene `better-auth/react` en un chunk lazy fuera del First Load JS del funnel; los bookers logueados nunca lo cargan. Sin CLS: el bloque se monta dentro del mismo `<section id="section-4">` bordeado; el flip a payment es un re-render server-side sin salto de layout observable.
- Tests (Playwright):
  - [x] `e2e/f-119-embedded-login.spec.ts` (3 tests, verdes contra `next dev`): (1) anónimo en Step 4 ve el bloque (Google+email+password+magic) y **cero** `a[href*="/login"]`; (2) email+pwd usuario nuevo auto-provisiona y voltea Section 4 a payment **sin navegar**; (3) magic link con email vacío → validación inline, no navega.
  - [x] Regresión: `e2e/f-049-spa.spec.ts` + `e2e/reservation-flow.spec.ts` re-escritos (aseveraban el CTA→`/login`, ahora aseveran el bloque embebido). 12/12 verdes. `f-041`/`f-027` (que citaban el viejo `step4-anonymous-cta`) están `describe.skip` con un esquema `/reservar/step-N` obsoleto — no corren, se dejan intactos.
  - Email+pwd **usuario existente** y magic link **"enviado"** / Google OAuth: caminos que dependen de credenciales pre-sembradas o servicios externos (Resend/Google). Cubiertos por lógica (fallback `USER_ALREADY_EXISTS`→signin) + aserción de wiring/`callbackURL`, no por E2E dedicado — dejar así.
- Notas:
  - Gotcha de test (memoria `verify-against-next-start-gotchas`): Better Auth **rate-limita signups en builds de prod** — E2E de auto-provisión usa `next dev` + emails únicos por run (`f119-${Date.now()}-…`).
  - `/login` intacto (aditivo): `f-005`/`f-033`/`f-050` no tocados. Redirects a `/login` de otras rutas protegidas (dashboard, exito, instructor/admin) inalterados.
  - Fuera de scope: rediseñar `/login`; passkeys; "guest checkout" sin cuenta (el producto requiere cuenta para dashboard/créditos).
- Entregado (2026-07-24): `app/[locale]/reservar/step4-auth.tsx` (nuevo), `page.tsx` (dynamic import + swap del bloque anónimo, `Link` retirado), `messages/{en,de,es}.json` (`login.continue`), `e2e/f-119-embedded-login.spec.ts` (nuevo), `e2e/{f-049-spa,reservation-flow}.spec.ts` (re-escritos). Typecheck sin errores nuevos (las 5 preexistentes de `f-104-a11y` por `@axe-core/playwright` no instalado son baseline, idénticas en primary). Lint limpio.
...
### F-122 — Email verificado antes de mantener un slot: cerrar el DoS de inventario por auto-provisión sin verificar

- Sprint: post-Sprint 5 (security hardening) · Estado: **done** (2026-08-01) · Prioridad: **P1 (seguridad — DoS de inventario de reservas)**
- Depende de: F-119 (embed login — email+password es el único método sin verificar), F-056 (account linking Google), F-060 (credits en el draft)
- Motivación: `lib/auth/index.ts` declara `emailAndPassword.enabled` **sin `requireEmailVerification`** → cualquiera auto-provisiona una cuenta con un email fake/no verificado por el path email+password (Google trae `email_verified`; magic link exige clic en el inbox — **solo email+password entra sin probar propiedad**). Y `lib/booking/create-draft.ts` exige sesión (L198) y **crea la fila `PENDING_PAYMENT` que ocupa el slot dentro de la transacción (L338) ANTES de crear el PaymentIntent (L502)**. El cron `expire-pending` (`vercel.json`: `*/15 * * * *`, TTL `PENDING_PAYMENT_EXPIRY_MS = 15 min`) solo libera el hold pasados ~15–30 min.
  - **Vector de DoS de inventario (reportado por owner 2026-07-28):** un bot auto-provisiona cuentas sin verificar **vía la API de Better Auth (sin tocar la UI)**, llama a `createBookingDraft` y mantiene todos los slots en `PENDING_PAYMENT` sin pagar nunca, re-holdeando según expiran. Ningún cliente real ve disponibilidad. Requiere **cero pago** porque el hold precede al PaymentIntent.
  - **Pre-existente y ortogonal a F-119:** el `/login` ya permitía signup email+password sin verificar; F-119 **no** abre el vector (un bot usa APIs, no el formulario embebido). F-119 se publica igual; este ticket endurece auth global + el chokepoint del hold.
- Objetivo: **exigir email verificado antes de que una cuenta pueda mantener un slot** — verificación antes incluso del `payment_intent`.
- AC — verificación de email (core):
  - [x] Activar verificación para el path password: `emailAndPassword.requireEmailVerification: true` + `emailVerification.sendVerificationEmail` (React Email + Resend, i18n en/de/es como `send-magic-link-email`). Google (trusted, `email_verified`) y magic link (marca `emailVerified` al usar) quedan **exentos**. Añadido `sendOnSignIn` (reenvía cuando una cuenta sin verificar intenta login) + `autoSignInAfterVerification` (el clic verifica **y** crea sesión, devolviendo al funnel con el draft).
  - [x] **Defense-in-depth en el chokepoint del hold:** `createBookingDraftWith` rechaza con nuevo error `EMAIL_NOT_VERIFIED` si `session.user.emailVerified !== true`, **antes** de la re-check de disponibilidad, la fila `PENDING_PAYMENT` y el PaymentIntent. `emailVerified` añadido al shape de sesión (`getSessionUser`) y al type de `CreateDraftDeps.session`.
  - [x] Funnel Step 4 (`step4-auth.tsx`): reescrito para `requireEmailVerification`. `signUp` ya no crea sesión y devuelve success genérico también para emails existentes (anti-enumeración), así que se resuelve el estado real con un `signIn` de seguimiento: ok → payment; `EMAIL_NOT_VERIFIED` → panel "confirma tu email" (con reenvío); inválido → nudge cuenta existente. `callbackURL` propagado para que el enlace de verificación vuelva al funnel. → email+password deja de ser 100% on-page — **trade-off aceptado (D-2026-07-28)**.
- AC — límite de daño (defense-in-depth adicional):
  - [x] Cap de holds `PENDING_PAYMENT` concurrentes por usuario (`MAX_CONCURRENT_HOLDS = 2`): `createBookingDraft` rechaza con `TOO_MANY_HOLDS` si el booker ya tiene 2 holds vivos (dentro de la ventana de expiry) → un actor no acapara múltiples slots aunque verifique. Corre tras el reuse idempotente, así que un resubmit del mismo slot no consume cuota.
  - [x] Rate-limit: Better Auth rate-limita signup + endpoints de auth en prod (confirmado); el cap de holds + la verificación son las capas nuevas. Por-IP explícito y acortar TTL quedan como margen si el abuso persiste (no necesarios para cerrar el vector).
- Tests:
  - [x] Unit `create-draft`: sesión `emailVerified:false` → `EMAIL_NOT_VERIFIED`, **no** crea fila ni PaymentIntent (ni carga pricing); `emailVerified:true` → happy path intacto; `TOO_MANY_HOLDS` al llegar al cap; hold permitido por debajo. 31 tests verdes en el archivo (531 en la suite completa).
  - [x] E2E `e2e/f-122-email-verification.spec.ts`: email+password nuevo → panel "confirma tu email" (no payment, sin salir del funnel) + reenvío confirma. `f-119-embedded-login.spec.ts` actualizado (los tests de flip-a-payment de email+password se movieron aquí; F-119 conserva embed + magic-link inline).
- Notas:
  - **Migración de usuarios existentes** (`20260801120000_f122_verify_existing_paid_bookers`): `UPDATE "User" SET emailVerified=true` para bookers con alguna reserva CONFIRMED/COMPLETED o con `paidAt` (pagaron → email real de facto). Idempotente (guard `emailVerified=false`). Aplicada a dev.
  - Fuera de scope: CAPTCHA; cambiar el modelo de hold-antes-de-pagar (el hold es correcto para no doble-vender; el fix es *quién* puede holdear, no *cuándo*).
- Refs: F-122, F-119, F-056, F-060, `lib/auth/index.ts`, `lib/auth/session-user.ts`, `lib/booking/create-draft.ts`, `lib/email/send-verification-email.tsx`, `app/(site)/[locale]/reservar/step4-auth.tsx`, `prisma/migrations/20260801120000_f122_verify_existing_paid_bookers`
...
- Alcance del daño, acotado: **las claves de Stripe en ese mismo fichero son de test** (`pk_test_…` / `sk_test_…`), así que el problema es de datos, no de dinero — desde local no se ha podido cobrar nada real.
- AC:
...
- AC — reenvío de webhooks de Stripe en local (detectado 2026-08-17 en la misma revisión):
  - El script **no falta**: `scripts/dev.mjs` sigue sincronizando `STRIPE_WEBHOOK_SECRET` y levantando `stripe listen --forward-to localhost:3000/api/webhooks/stripe` junto a `next dev`. Lo que faltaba en la máquina nueva era el **login de la CLI** — vive en `~/.config/stripe/config.toml`, fuera del repo, así que no viaja con el proyecto. Sin él, `stripe listen --print-secret` responde *"You have not configured API keys yet"*, `dev.mjs` entra por su rama fail-soft y arranca sólo Next. Se resuelve con `stripe login` una vez.
  - [ ] Documentar `stripe login` como paso de alta de máquina (README + `docs/WORKFLOW.md`), junto al de sembrar `.env.local`. Es el único requisito del entorno que no está ni en el repo ni en el fichero de env, y por eso se pierde en silencio al cambiar de ordenador.
  - [ ] Subir el volumen del fail-soft: hoy el aviso es una línea amarilla entre el ruido de arranque de Next. Que diga explícitamente que **los pagos locales no recibirán webhooks** hasta hacer `stripe login`, que es la consecuencia real.
  - [ ] Ojo al acoplamiento con este ticket: `dev.mjs` **reescribe** `.env.local` en cada arranque (`patchEnvLocal`). Cualquier automatización que se añada aquí tiene que convivir con eso — y conviene comprobar que no pisa el `DATABASE_URL` corregido.
- Notas:
...
- AC (orden por puntos):
  ...
  - [x] 2.7 (+4) grupal vs privada → **superseded por F-146** (`private-vs-ski-school` + `small-group-lessons`, páginas más ricas; stub `group-vs-private` eliminado en el PR de F-146)
  - [x] 2.8 (+3) elegir instructor privado → **superseded por F-146** (`choosing-lessons` criterio 3 + `your-instructor`; stub `choosing-instructor` eliminado en el PR de F-146)
  ...
  - [x] 2.11 (+3) precio de clase particular por hora → **superseded por F-146** (`full-day-price`, con lista completa de precios; stub `private-prices` eliminado en el PR de F-146)
  - [x] 2.12 (+3) mejorar técnica de carving con instructor — `advanced-coaching`
  - [x] 2.13 (+2) clases particulares para niños → **superseded por F-146** (`kids-private-lessons`; stub `kids-families` eliminado en el PR de F-146)
  - [x] 2.14 (+1) grupos pequeños → **superseded por F-146** (`small-group-lessons`, página dedicada)
  - [x] 2.16 (+1) familias con niños → **superseded por F-146** (`family-lessons`)
  - [x] 2.18 (+0,7) día completo → **superseded por F-146** (`full-day-price`)
...
- **Entregado (2026-09-18):** patrón answer-first + páginas answer-first como posts del blog (decisión del owner) reutilizando `blog/[slug]/page.tsx`. Mecanismo: frontmatter opcional `faq: [{q,a}]` en el MDX → `lib/blog/posts.ts` lo valida/transporta y la página emite además `FAQPage` JSON-LD vía `buildFaqPage` junto al `BlogPosting`. Sin cambios de routing; sitemap/hreflang/OG ya cubren los posts. **Retenidas por F-145** (`content/blog/{es,de,en}/`, id compartido): `adult-levels` (1.2), `advanced-coaching` (2.9+2.12), `learn-options` (2.10). **Superseded por F-146** (páginas más ricas; los stubs de F-145 se eliminan en el PR de F-146): `group-vs-private`→`private-vs-ski-school`+`small-group-lessons` (2.7/2.14), `choosing-instructor`→`choosing-lessons`+`your-instructor` (2.8), `private-prices`→`full-day-price` (2.11/2.18), `kids-families`→`family-lessons`+`kids-private-lessons` (2.13/2.16). Tests: `e2e/f-145-answer-first.spec.ts` (3 posts retenidos × 3 locales tras el prune de F-146). **Pendiente:** 2.2 (bloque Q&A UI) y 1.3 (reestructura de precios/privadas) → follow-up; 2.15/2.17 (reseñas) → ahora cubiertas honestamente por `your-instructor` de F-146 (sin reseñas inventadas).

### F-146 — Ganar las consultas donde aparecen los rivales, por méritos propios (+14 pt)

- Sprint: 7 · Estado: done · Prioridad: P1
- Depende de: F-145 (comparte el patrón answer-first + infra `faq`/`FAQPage`; este PR va **apilado sobre `f-145-answer-first-qa`**, PR #214, y supersede 4 de sus stubs)
...
- **Supersede de F-145 (decisión del owner, 2026-09-18):** al implementarse, el PR abierto de F-145 (#214) ya había expandido a los mismos clusters con stubs cortos. Este PR (apilado sobre `f-145-answer-first-qa`) **elimina 4 stubs de F-145** — `group-vs-private`, `choosing-instructor`, `private-prices`, `kids-families` (×3 locales = 12 MDX) — y poda sus entradas en `e2e/f-145-answer-first.spec.ts`, dejando a F-145 con `adult-levels`, `learn-options` y `advanced-coaching`. Las páginas ricas de F-146 pasan a servir también las consultas 2.7/2.8/2.11/2.13/2.14/2.16/2.18 (antes de F-145) y 2.15/2.17 (reseñas, que F-145 había diferido a F-149) vía `your-instructor` sin reseñas inventadas. Nota de cobertura: la consulta exacta "diferencia entre clase grupal y privada" (2.7) queda servida por `private-vs-ski-school`; si el owner quiere el framing literal grupal-vs-privada, añadir una sub-Q a ese post.
