...
- AC layout:
  ...
  - [x] **Decisión comercial del owner (2026-07-24):** **Contact fuera del nav** (dropdown + `MobileNav`) — sigue accesible desde el footer (`footer-contact-link`) + el CTA de teléfono de la utility bar. Y **eliminado el botón "Sign in" del hero del home** (antes condicional por sesión en F-107; ahora fuera para todo el mundo) para reducir la fricción hacia la reserva — el hero deja un único CTA "Book a lesson". Sign-in sigue en el `SiteNav`. Se quita la lectura de sesión del home page (imports `auth`/`headers`). Memoria de proyecto `hero-no-signin-reduce-booking-friction`. Specs actualizados: f-107 issue 1 (hero sin sign-in), f-116/f-096/f-102 (Contact ya no en nav → footer).
